...
const UDP_IPV4_ORIGIN: u8 = 0;
const UDP_IPV6_ORIGIN: u8 = 1;
...
pub fn to_datagram(socket_addr: SocketAddr, data: &[u8], data_size: usize) -> Bytes {
    let mut bytes: BytesMut = match socket_addr.ip() {
        IpAddr::V4(v4) => {
            let mut bytes = BytesMut::with_capacity(UDP_HEADER_IPV4_LEN + data_size);
        ...
        IpAddr::V6(v6) => {
            let mut bytes = BytesMut::with_capacity(UDP_HEADER_IPV6_LEN + data_size);
